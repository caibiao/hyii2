...
//                $op = ['data-toggle'=>'tooltip','data-original-title'=>$item['label']];
//                $op['class'] = 'tooltips '.$active;
//                $html .= Html::tag('li',Html::tag('a',$item['label'],['data-toggle'=>'tab','data-target'=>'#tab-'.$k]),$op)."\n";
                $leftMenu .= '<div class="tab-pane '.$active.'" id="tab-'.$k.'">';
                $leftMenu .= '<h5 class="sidebar-title">菜单</h5>';
                if(isset($item['items'])){
                    foreach ($item['items'] as &$lt){
                        if(!isset($lt['options']) && !empty($lt['items'])){
                            $lt['options']['class'] = 'nav-parent';
                        }
                    }
                    $leftMenu .= Html::tag($tag, $this->renderItems($item['items']), $options);
                }
                $leftMenu .= '</div>';
            }
            //Html::tag('ul', $html, ['class'=>'nav nav-tabs nav-justified nav-sidebar']);
            echo Html::tag('div', $leftMenu, ['class'=>'tab-content'] );
        }
    }
